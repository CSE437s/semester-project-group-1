    ...
    return(
        <div className="flex flex-row items-center -m-10">
            {props.data === undefined ? <></> : <>
                <div className="flex flex-row justify-between items-center w-auto bg-white rounded-lg shadow-md px-4 py-2">
                    <div className="flex flex-row items-center text-black font-light text-sm">
                        ...
                        <p className="">Date Range Start: {cropISOString(props.data.beginRangeSearch.toISOString())}</p>
                        <p className="mx-2">Date Range End: {cropISOString(props.data.endRangeSearch.toISOString())}</p>
                    </div>
                </div>
                <Button onClick={() => props.setExpanded(true)} className="mx-2 px-2 text-sm font-semibold h-12 bg-white text-black">Expand</Button>
                </>
            }
        </div>
